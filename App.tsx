...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const storedUser = localStorage.getItem('st_user_profile');
        if (storedUser) {
    ...
    const handleThemeChange = (newTheme: Theme) => {
        ...
        if (user) {
            ...
            setUser(updatedUser);
            localStorage.setItem('st_user_profile', JSON.stringify(updatedUser));
        }
    };

    const handleLogin = (newUser: UserProfile) => {
        setUser(newUser);
        localStorage.setItem('st_user_profile', JSON.stringify(newUser));
    ...
    const handleUpdateUser = (updatedUser: UserProfile) => {
        setUser(updatedUser);
        localStorage.setItem('st_user_profile', JSON.stringify(updatedUser));
        // Note: We don't necessarily need to change view, maybe just show a success toast inside ProfileView
        // But for now, we can stay on profile or go back. Let's stay on profile.
    };
    ...
    const handleLogout = () => {
        setUser(null);
        localStorage.removeItem('st_user_profile');
        setCurrentView('welcome');
    };
    ...
    const handleSaveLesson = (plan: LessonPlan) => {
        try {
            const stored = localStorage.getItem('st_resources');
            const existing: Resource[] = stored ? JSON.parse(stored) : [];
            // Avoid duplicate saves
            if (existing.some(r => r.type === 'lesson-plan' && r.data?.id === plan.id)) {
                alert('هذا الدرس محفوظ بالفعل في المصادر!');
                return;
            }
            const newResource: Resource = {
                ...
                title: plan.topic,
                type: 'lesson-plan',
                tags: [plan.subject, plan.grade].filter(Boolean) as string[],
                createdAt: new Date().toISOString(),
                data: { ...plan, assets: plan.assets },
            };
            existing.push(newResource);
            localStorage.setItem('st_resources', JSON.stringify(existing));
            alert('✅ تم حفظ الدرس في المصادر والمناهج بنجاح!');
        } catch (e) {
        ...
        switch (currentView) {
            ...
            case 'dashboard':
                return user ? (
                    ...
                    />
                ) : <WelcomeScreen onStartLogin={() => setCurrentView('login')} onGuestAccess={handleGuestAccess} />;
            case 'create-lesson':
                return (
                    <LessonGenerator
